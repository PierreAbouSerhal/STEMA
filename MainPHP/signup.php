    ...
    if(isset($_POST["signup"]))
    {
        ...
        if(!empty($usrName) && !empty($usrPhone) && !empty($usrEmail) && !empty($usrPass1) && !empty($usrPass2))
        {
            if(is_numeric($usrPhone) && strpos($usrEmail, '@') !== false && strlen($usrPass1) > 4 && $usrPass1 == $usrPass2)
            {
                ...
                $html = '<h1>Hi '.$usrName.'</h1><p>Thanks for getting started with Stema! Please click on the link below to confirm your email address: <a href ="http://localhost/STEMA/MainPhp/activate.php?activCode='.$activCode.'">Activate account</a></p>
